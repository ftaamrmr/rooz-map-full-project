...
@router.post("/analyze")
async def analyze_competitor(url: str, mode: str | None = None, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    if not settings.SCRAPER_SERVICE_URL:
        raise HTTPException(status_code=500, detail="Scraper service URL is not configured. # TODO: # ضع SCRAPER_SERVICE_URL في env أو في Coolify")
